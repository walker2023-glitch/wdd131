...
// FIXED: Hook up click and enter key listeners so the system responds
if (button) button.addEventListener('click', search);
if (input) input.addEventListener('keypress', handleEnter);

function search() {
    // FIXED: Renamed to hikeQuery to perfectly align with your filter snippet parameters
    let hikeQuery = input.value;
    
    let filterRecipes = recipes.filter(function(hike){
        return (
    ...
    // Save social icons element reference
    let socialDiv = document.querySelector('.social');
    recipeContainer.innerHTML = ''; 

    // Sort by name alphabetically using clean function scope
    let sortRecipes = filterRecipes.sort(compareRecipes);

    // Render results
    sortRecipes.forEach(function(recipe){
        renderRecipe(recipe);
    });

    // Re-append social links at the bottom
    if (socialDiv) recipeContainer.appendChild(socialDiv);
}

// Alphabetical cleaning function
...
function renderRecipe(recipe) {
    ...
    let socialDiv = document.querySelector('.social');
    if (socialDiv) {
        socialDiv.insertAdjacentHTML('beforebegin', html);
    } else {
